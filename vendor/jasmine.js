...
ConsoleReporter.prototype = {
	...
	if (this.hasGroupedConsole()) {
	    ...
	    // enable propagation
	    Inio.connector.catchLogs = false;
	}
	else {
	    ...
	    this.log("Jasmine Finished.");
	    this.log(spec_str + fail_str + (dur / 1000) + "s.");
	}
	this.finished = true;
    },
    hasGroupedConsole: function() {
	return (Inio && Inio.connector && Inio.connector.connected);
    },
	...
	if (!this.hasGroupedConsole()) {
	    ...
	    // prevent propagation to the Inio.debug
	    Inio.connector.catchLogs = true;
	}
	
	this.log("Jasmine Started...");
    },
    reportSpecResults: function(spec) {
	var results = spec.results();

	if (!this.hasGroupedConsole()) {
	    var resultText = "[Failed -> "+(spec.suite ? spec.suite.description+" : " : "")+spec.description+"]";

	    if (spec.results().passed()) {
		this.passed_specs++;
		resultText = "[Passed -> "+(spec.suite ? spec.suite.description+" : " : "")+spec.description+"]";
		
	    }else{
		for(var i in results.items_){
		    if(results.items_[i].message && results.items_[i].message !== 'Passed.'){
			resultText += ' '+results.items_[i].message;
			break;
		    }
		}
	    }
...
function itemResults(item) {
	...
	console.warn(item.matcherName, item.message);
	//console.dir(item);
